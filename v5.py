...
def runSimulation(evt):
    ...
    if evt.text == 'Run Simulation':
        run = True      
runButton = button(bind = runSimulation, text = 'Run Simulation', disabled = True)
...
torqueGraph = graph(title = 'Torque vs. Angle',xmin = 0,xmax = 2*pi)
torqueCurve = gcurve(graph=torqueGraph, color=color.red, label="Torque")

fieldGraph = graph(title = 'Electric Field vs. Distance', xmin = 0)
dipoleFieldCurve = gcurve(graph=fieldGraph, color = color.red, label='Electric Field from Dipole')
fieldCurve = gcurve(graph=fieldGraph, color=color.blue, label = 'Electric Field from a Charged Particle')
...
while True:
    ...
    if dipoleVisualization == True:
        ...
        xposslider.disabled = True
        yposslider.disabled = True
        arrowShowButton.disabled = True
        chlorine.disabled = True
        water.disabled = True
        runButton.disabled = True
        generateButton.disabled = True
        dipoleVisualizationButton.text = 'To exit Dipole Visualization Mode, please refresh the page'
